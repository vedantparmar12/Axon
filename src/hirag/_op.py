...
async def _build_hierarchical_query_context(
    ...
    def find_path_with_required_nodes(graph, source, target, required_nodes):
        # inital final path
        final_path = []
        # Set the starting point as the current node
        current_node = source

        # Iterate through required nodes
        for next_node in required_nodes:
            # Find the shortest path from current node to the next required node
            try:
            ...
            # Merge paths (avoid duplicate addition of current node)
            if final_path:
                final_path.extend(sub_path[1:])  # Start adding from the second node to avoid duplication
            else:
                final_path.extend(sub_path)

            # Update current node to the next required node
            current_node = next_node

        # Finally, path from the last required node to the target node
        try:
