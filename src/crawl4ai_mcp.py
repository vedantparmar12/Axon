...
@mcp.tool()
async def perform_light_rag_query(ctx: Context, query: str, source: str = None, match_count: int = 5, low_level_match_count: int = 20) -> str:
    """
    Perform a light RAG query using a two-stage hybrid approach (low-level and high-level).

    This tool first performs a broad, low-level retrieval using both keyword and vector search
    to gather a larger pool of candidate documents. It then uses a more sophisticated
    cross-encoder model for high-level reranking to provide highly relevant results.

    Args:
        ctx: The MCP server provided context
        query: The search query
        source: Optional source domain to filter results
        match_count: The final number of results to return after reranking (default: 5)
        low_level_match_count: The number of candidates to fetch in the low-level retrieval step (default: 20)
    ...
    try:
        ...
        if not reranking_model:
            return json.dumps({
                "success": False,
                "query": query,
                "error": "Reranking model is not available, which is required for light RAG. Set USE_RERANKING=true."
            }, indent=2)
        ...
        vector_results = search_documents(
            client=supabase_client,
            query=query,
            match_count=low_level_match_count,
            filter_metadata=filter_metadata
        )

        # 2. Keyword search
        keyword_query_builder = supabase_client.from_('crawled_pages').select('id, url, chunk_number, content, metadata, source_id')
        keyword_query_builder = keyword_query_builder.ilike('content', f'%{query}%')
        if source and source.strip():
            keyword_query_builder = keyword_query_builder.eq('source_id', source)

        keyword_response = keyword_query_builder.limit(low_level_match_count).execute()
        keyword_results = keyword_response.data if keyword_response.data else []

        # 3. Combine and deduplicate
        combined_results = {r['id']: r for r in vector_results}
        for r in keyword_results:
            if r['id'] not in combined_results:
        ...
        # --- High-level retrieval (reranking) ---
        reranked_results = rerank_results(reranking_model, query, candidate_docs, content_key="content")
        ...
        for result in final_results:
            formatted_result = {
                ...
                "content": result.get("content"),
                "metadata": result.get("metadata"),
                "similarity": result.get("similarity"), # from vector search, might be None
        ...
        return json.dumps({
            ...
            "query": query,
            "source_filter": source,
            "search_mode": "light_rag",
            "reranking_applied": True,
            "low_level_candidates": len(candidate_docs),
            "results": formatted_results,
            "count": len(formatted_results)
        }, indent=2)
