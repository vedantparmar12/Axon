...
@mcp.tool()
async def hirag_query(ctx: Context, query: str, mode: str = "hi", top_k: int = 10) -> str:
    """
    Perform a hierarchical RAG query using hiRAG for advanced entity-based retrieval.

    This tool uses hierarchical knowledge graphs and entity clustering to provide
    more comprehensive and contextual answers compared to standard RAG.

    Args:
        ctx: The MCP server provided context
        query: The search query
        mode: Query mode - options: hi, hi_global, hi_local, hi_bridge, hi_nobridge, naive
        top_k: Number of results to return (default: 10)

    Returns:
        JSON string with hierarchical search results
    """
    try:
        hirag_provider = ctx.request_context.lifespan_context.hirag_provider

        if not hirag_provider:
            return json.dumps({
                "success": False,
                "error": "hiRAG provider not initialized. Set USE_HIRAG=true in environment."
            })

        # Perform hierarchical search
        results = await hirag_provider.search(query, top_k=top_k, mode=mode)
        ...
        return json.dumps({
            "success": False,
            "query": query,
            "error": f"hiRAG query failed: {str(e)}"
        }, indent=2)

@mcp.tool()
async def hirag_get_entity_graph(ctx: Context, query: str = None) -> str:
    """
    Get the entity knowledge graph structure from hiRAG.

    This tool returns the entities and relationships discovered by hiRAG
    ...
    Returns:
        JSON string with graph nodes and edges
    """
    try:
        hirag_provider = ctx.request_context.lifespan_context.hirag_provider
        ...
        # Get entity graph
        graph_data = await hirag_provider.get_entity_graph(query)
        ...
        return json.dumps({
            "success": False,
            "error": f"Failed to get entity graph: {str(e)}"
        }, indent=2)

@mcp.tool()
async def hirag_get_communities(ctx: Context) -> str:
    """
    Get the detected communities from hiRAG's entity clustering.

    This tool returns the communities/clusters of entities discovered by hiRAG
    along with their summaries and member entities.
    ...
    try:
        ...
        # Get communities
        communities = await hirag_provider.get_communities()
        ...
        return json.dumps({
            "success": False,
            "error": f"Failed to get communities: {str(e)}"
        }, indent=2)

@mcp.tool()
async def hirag_add_documents(ctx: Context, documents: List[Dict[str, Any]]) -> str:
    """
    Add documents to hiRAG for hierarchical processing and entity extraction.

    This tool processes documents through hiRAG's pipeline to extract entities,
    build knowledge graphs, and create hierarchical clusters.

    Args:
        ctx: The MCP server provided context
        documents: List of documents, each with 'id', 'content', and 'metadata' keys
    ...
    try:
        ...
        # Add documents to hiRAG
        await hirag_provider.add_documents(documents)

        return json.dumps({
            "success": True,
            "message": f"Successfully added {len(documents)} documents to hiRAG",
            "count": len(documents)
        }, indent=2)
        ...
        return json.dumps({
            "success": False,
            "error": f"Failed to add documents to hiRAG: {str(e)}"
        }, indent=2)

@mcp.tool()
async def hirag_clear_cache(ctx: Context) -> str:
    """
    Clear hiRAG cache and reinitialize the knowledge graph.

    This tool clears all cached data in hiRAG including entities,
    ...
    try:
        ...
        # Clear cache
        await hirag_provider.clear_cache()

        return json.dumps({
            "success": True,
            "message": "hiRAG cache cleared successfully"
        }, indent=2)

    except Exception as e:
        return json.dumps({
            "success": False,
            "error": f"Failed to clear hiRAG cache: {str(e)}"
        }, indent=2)

@mcp.tool()
async def hirag_get_stats(ctx: Context) -> str:
    """
    Get hiRAG system statistics and configuration.

    This tool returns information about hiRAG's current configuration
    ...
    try:
        ...
        # Get stats
        stats = hirag_provider.get_stats()
        ...
        return json.dumps({
            "success": False,
            "error": f"Failed to get hiRAG stats: {str(e)}"
        }, indent=2)
