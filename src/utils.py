...
def create_embeddings_batch(texts: List[str]) -> List[List[float]]:
    ...
    # Use async embedding manager
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        embeddings = loop.run_until_complete(embedding_manager.create_embeddings_batch(texts))
        return embeddings
    finally:
        loop.close()
...
def create_embedding(text: str) -> List[float]:
    ...
    asyncio.set_event_loop(loop)
    try:
        embedding = loop.run_until_complete(embedding_manager.create_embedding(text))
        return embedding
    finally:
        loop.close()
...
Please give a short succinct context to situate this chunk within the overall document for the purposes of improving search retrieval of the chunk. Answer only with the succinct context and nothing else."""
        ...
        system_prompt = f"""You are a helpful assistant that provides concise contextual information.
...
def generate_code_example_summary(code: str, context_before: str, context_after: str) -> str:
    ...
    # Create the prompt
    prompt = "Based on the code example and its surrounding context, provide a concise summary (2-3 sentences) that describes what this code example demonstrates and its purpose. Focus on the practical application and key concepts illustrated."
    
    system_prompt = f"""You are a helpful assistant that provides concise code example summaries.
...
def extract_source_summary(source_id: str, content: str, max_length: int = 500) -> str:
    ...
    prompt = f"The content is from the documentation for '{source_id}'. Please provide a concise summary (3-5 sentences) that describes what this library/tool/framework is about. The summary should help understand what the library/tool/framework accomplishes and the purpose."
    
    system_prompt = f"""You are a helpful assistant that provides concise library/tool/framework summaries.
